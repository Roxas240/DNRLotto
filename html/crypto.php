...
switch ($method) {
    case "requests":
            $query = "SELECT id, amount, filled, notes FROM $table WHERE (code='-1')";
	...
	case "linktix":
	        $query  = "SELECT `value` FROM `$ltt_table` WHERE name='sold-tickets'";
			$result = mysqli_query($linkDB, $query) or die("Error: ".mysqli_error($linkDB));
			$dat    = $result->fetch_assoc();
			$ticket_id = $dat['value'];
			
			$query  = "SELECT `value` FROM `$ltt_table` WHERE name='lotto-count'";
			$result = mysqli_query($linkDB, $query) or die("Error: ".mysqli_error($linkDB));
			$dat    = $result->fetch_assoc();
			$round_id = $dat['value'];
	
			$query   = "INSERT INTO `lotto_tickets` (payment_id, address, txid, ticket_id, round_id) VALUES ('$payID', '$address', '$txid', '$ticket_id', '$round_id')";
            $result  = mysqli_query($conn, $query) or die(mysqli_error($conn));
			$newID = mysqli_insert_id($linkDB);
			echo $newID.":".$ticket_id.":".$round_id;
			
			$ticket_id += 1;
			
			$query = "UPDATE $ltt_table SET value='$ticket_id' WHERE name='sold-tickets'";
            $result = mysqli_query($linkDB, $query) or die("Error: ".mysqli_error($linkDB));
			
			$pot = $ticket_id * 1;
			$query = "UPDATE $ltt_table SET value='$pot' WHERE name='pot-total'";
            $result = mysqli_query($linkDB, $query) or die("Error: ".mysqli_error($linkDB));
        break;
    default:
        echo "Error:-1";
