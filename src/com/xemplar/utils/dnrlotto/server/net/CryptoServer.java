...
import com.xemplar.utils.dnrlotto.server.NodeProps;
import com.xemplar.utils.dnrlotto.server.ServerListener;
        ...
        for(Request r : requests){
            payments.add(new PaymentHandler(this, r, new NetworkListener.ServerPaymentListener() {
                public void paymentReceived(PaymentHandler handler, String txID) {
                    ...
                    params.clear();
                    params.add(new BasicNameValuePair("payID", handler.getID() + ""));
                    params.add(new BasicNameValuePair("txid", txID));
                    params.add(new BasicNameValuePair("address", handler.getAddress()));
                    CryptoServer.this.link.doPost("linktix", params, null);
                ...
                public void paymentCanceled(PaymentHandler handler) {
                    List<NameValuePair> params = new ArrayList<NameValuePair>();
                    params.add(new BasicNameValuePair("id", handler.getID() + ""));
                    CryptoServer.this.link.doPost("cancel", params, null);
