package com.xemplar.utils.dnrlotto.server.net;

import com.xemplar.utils.dnrlotto.common.NetworkListener;
import com.xemplar.utils.dnrlotto.server.domain.Transaction;
import com.xemplar.utils.dnrlotto.server.net.domain.Request;
...
import static com.xemplar.utils.dnrlotto.server.net.CryptoServer.MIN_CONFIRMS;

/**
 * Created by Rohan on 8/20/2017.
 */
public class PaymentHandler implements Runnable{
    public static long PAYMENT_TIMEOUT = 900000;
    ...
    public PaymentHandler(CryptoServer rest, Request req, NetworkListener.ServerPaymentListener listener){
        ...
        this.request = req;
        this.id = req.id;
        this.address = req.notes.split(":")[1];
        this.canceled = false;
    }
    ...
    public void run() {
        ...
        while (!paid && !canceled) {
            ...
            for (String id : allTX) {
                ...
                if(confirm > 0) {
                    totalSent += Double.parseDouble(b.getAmount().toPlainString());
            ...
            if(totalSent >= Double.parseDouble(price.toPlainString())){
                this.millisStarted = System.currentTimeMillis();
            }

            if(this.millisStarted + PAYMENT_TIMEOUT <= System.currentTimeMillis()){
                cancel();
                System.out.println("Payment " + id + " has timed out");
            }
